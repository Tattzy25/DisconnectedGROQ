...
// Legacy Redis functions - kept for backward compatibility but no longer used in new code
// These functions are still imported by some API routes that haven't been fully migrated
...
// Legacy Redis functions - kept for backward compatibility
...
// Legacy Redis functions - kept for backward compatibility
export async function getFromStorageWithRegex(key: string): Promise<{value: string | null, key: string}> {
	const redis = Redis.fromEnv();
	...
	return {value: value as string | null, key: keys[0]};
}
